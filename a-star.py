...
class PriorityQueue:
    ...
    def put(self, node, distance, priority):
        heapq.heappush(self.elements, (priority, node, distance))
...
def parse_input(input_text):
    ...
    for line in lines[1:1+citiesNb]:
        ...
        print(f"Added city {city} with coordinates {cities[city]}")
...
def astar(cities, graph, start, goal):
    ...
    pq = PriorityQueue()
    # City, Distance, Priority
    pq.put('Tokyo', 1.0, 4.0)
    pq.put('Toyama', 2.0, 3.0)
    pq.put('Kyoto', 7.0, 8.0)

    while not pq.empty():
       node = pq.get()
       print(f"Retrieved {node[1]} with priority {node[0]} and distance {node[2]}")
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("Usage: python astar.py input")
    else:
        main(sys.argv[1])
